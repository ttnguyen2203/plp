...
class TestPigLatinMapper {
    ...
    void testInvalidWords() {
        final Map<String, Object > inputs = new HashMap<>();
        inputs.put("a", "1Happy"); // rule: start with consonant + vowel
        inputs.put("b", "C4hild"); // rule: start with 2 consonants
        inputs.put("c", "Awesodsfame"); // rule: start with a vowel
        ...
        Assert.isTrue(Objects.equals(result.get("c"), "Awesodsfame"));
    }
    ...
    void testSentences() {
        ...
        Assert.isTrue(result.size() == 3, "Same number of entries as input");
        Assert.isTrue(Objects.equals(result.get("a"), "asyay ayay embermay ofyay ethay eekgray ommunitycay"));
        Assert.isTrue(Objects.equals(result.get("b"), "As1 ayay m3mb3r ofyay eezay Gr33k comminit!"));
    }
}